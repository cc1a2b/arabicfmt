...
describe("transliterate", () => {
  it("romanizes consonants (bare text is consonant-only)", () => {
    ...
    expect(transliterate("الرياض")).toBe("alryad");
  });
...
describe("slugify", () => {
  it("produces URL-safe slugs", () => {
    expect(slugify("مدينة جدة")).toBe("mdynh-jdh");
    expect(slugify("الرياض 2026")).toBe("alryad-2026");
  });
  ...
  it("trims and collapses separators", () => {
    expect(slugify("  جدة،،، مدينة  ")).toBe("jdh-mdynh");
  });
});
