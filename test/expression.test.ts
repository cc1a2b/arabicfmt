...
describe("spellCurrency — precision varies by currency", () => {
  ...
  it("SDG pounds", () => {
    expect(spellCurrency(5, { currency: "SDG" })).toBe("خمسة جنيهات");
  });
...
describe("spellCurrency — options", () => {
  ...
  it("suffix true adds the cheque ending", () => {
    expect(spellCurrency(100, { currency: "SAR", suffix: true })).toBe(
      "مئة ريال فقط لا غير",
    );
  });

  it("custom suffix", () => {
    expect(spellCurrency(100, { currency: "SAR", suffix: "فقط" })).toBe(
      "مئة ريال فقط",
    );
  });
